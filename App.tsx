...
const App: React.FC = () => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      ...
      alert('링크가 복사되었습니다. 카카오톡이나 SNS에 붙여넣어 공유해주세요!');
    }
  };
  ...
  return (
    <div className="flex justify-center items-center w-full h-[100dvh] bg-gray-200 font-sans overflow-hidden p-0 sm:p-4">
      ...
      >
        ...
        {/* 텍스트 컨텐츠 영역 */}
        <div className={`flex-1 flex flex-col px-6 ${currentIndex >= 3 ? 'pt-6 sm:pt-10 pb-2' : 'pt-6 sm:pt-8 pb-3'} bg-white min-h-0 overflow-hidden transition-all`}>
          
          {currentCard.keyword && currentIndex !== 0 && (
            <div className={`${currentIndex >= 3 ? 'mb-2 sm:mb-4' : 'mb-4 sm:mb-6'} flex-shrink-0 transition-all`}>
              <span className="inline-block px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 text-[12px] sm:text-[13px] font-black border border-emerald-100 tracking-tight">
                {currentCard.keyword}
              </span>
            </div>
          )}
          ...
          <div className={`${currentIndex === 0 ? 'mt-2' : ''} flex-shrink-0`}>
            <h1 className={`${currentIndex >= 3 ? 'text-[clamp(1.05rem,3.5vh,1.25rem)]' : 'text-[clamp(1.05rem,3.8vh,1.35rem)]'} font-black text-gray-900 leading-[1.3] mb-3 sm:mb-4 whitespace-pre-wrap tracking-tight break-keep`}>
              {currentCard.title}
            </h1>
          </div>

          <div className={`flex-1 flex flex-col no-scrollbar overflow-y-auto ${currentIndex >= 3 ? 'space-y-3 sm:space-y-4' : 'space-y-2 sm:space-y-2.5'}`}>
            {currentCard.subtitle && (
              <p className="text-emerald-700 font-bold text-[12.5px] sm:text-[13.5px] leading-relaxed border-l-[3px] border-emerald-500 pl-3 break-keep">
                {currentCard.subtitle}
              </p>
            )}
            {currentCard.body && (
              <p className="text-gray-600 text-[clamp(0.8rem,1.9vh,0.95rem)] leading-[1.6] sm:leading-[1.65] font-medium whitespace-pre-wrap break-keep">
                {currentCard.body}
              </p>
            )}
            ...
            {currentCard.buttonText && !isLastPage && (
              <div className="pt-1">
                <span className="inline-block bg-yellow-400 text-yellow-900 px-4 py-1.5 rounded-full font-bold text-[10px] sm:text-[11px] shadow-sm animate-bounce">
            ...
            {currentCard.keyword && currentIndex === 0 && (
              <div className="pt-3 sm:pt-4 pb-2 transition-all">
                <span className="inline-block px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 text-[12px] sm:text-[13px] font-black border border-emerald-100 tracking-tight">
                  {currentCard.keyword}
                </span>
              </div>
          ...
          {/* 마지막 페이지 버튼들 */}
          {isLastPage && (
            <div className="mt-auto pt-2 mb-4 flex-shrink-0 flex flex-col items-center gap-2">
              <button 
                onClick={() => window.open(CARD_NEWS_CONFIG.links.donation, '_blank')}
                className="w-full max-w-[320px] bg-emerald-500 text-white py-3 sm:py-3.5 rounded-xl font-black text-[15px] sm:text-[16px] flex flex-col items-center justify-center gap-0.5 shadow-lg active:scale-95 transition-transform"
              >
                <div className="flex items-center gap-2">
                  <Heart className="w-4 h-4 sm:w-5 sm:h-5 fill-current" />
                  <span>지금 바로 정기 후원하기</span>
                </div>
                <span className="text-[9px] sm:text-[10px] font-medium opacity-90">발달장애인의 내일을 지원합니다</span>
              </button>
              
              <button 
                onClick={handleShare}
                className="w-full max-w-[320px] bg-white border-2 border-emerald-500 text-emerald-600 py-2 sm:py-2.5 rounded-xl font-bold text-[13px] sm:text-[14px] flex items-center justify-center gap-2 active:bg-emerald-50 transition-colors"
              >
                <Share2 size={16} />
                주변에 소식 공유하기
              </button>
